/* eslint-disable @typescript-eslint/no-explicit-any, react-hooks/exhaustive-deps */
import React, { useEffect, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { useFlowStore } from '@/store/flowStore';
import { Card, CardHeader, CardBody, Badge, Button, Modal, Alert } from '@/components';
import { Plus, Pencil, Trash2, Power, PowerOff } from 'lucide-react';

export const FlowListPage: React.FC = () => {
  const navigate = useNavigate();
  const {
    flows,
    isLoading,
    isSaving,
    error,
    fetchFlows,
    activateFlow,
    deactivateFlow,
    deleteFlow,
    clearError,
  } = useFlowStore();

  const [deleteTarget, setDeleteTarget] = useState<{ id: string; name: string; isActive: boolean } | null>(null);
  const [invalidRefError, setInvalidRefError] = useState<string | null>(null);

  useEffect(() => {
    void fetchFlows();
  }, []);

  const handleActivate = async (id: string) => {
    try {
      await activateFlow(id);
    } catch (err: any) {
      surfaceInvalidRefError(err);
    }
  };

  const handleDeactivate = async (id: string) => {
    try {
      await deactivateFlow(id);
    } catch (err: any) {
      surfaceInvalidRefError(err);
    }
  };

  const handleDeleteClick = (flow: { id: string; name: string; isActive: boolean }) => {
    setDeleteTarget(flow);
  };

  const handleConfirmDelete = async () => {
    if (!deleteTarget) return;
    try {
      await deleteFlow(deleteTarget.id);
      setDeleteTarget(null);
    } catch (err: any) {
      surfaceInvalidRefError(err);
      setDeleteTarget(null);
    }
  };

  /** Surface INVALID_FLOW_REFERENCE errors with offending ids */
  const surfaceInvalidRefError = (err: any) => {
    const data = err?.response?.data;
    if (data?.error?.code === 'INVALID_FLOW_REFERENCE' || data?.code === 'INVALID_FLOW_REFERENCE') {
      const ids = data?.error?.invalidIds || data?.invalidIds || [];
      const idsStr = Array.isArray(ids) && ids.length > 0
        ? ids.join(', ')
        : 'unknown';
      setInvalidRefError(`Invalid flow reference — offending IDs: ${idsStr}`);
    }
  };

  const formatDate = (dateStr: string) => {
    const d = new Date(dateStr);
    return d.toLocaleDateString('en-ZA', {
      year: 'numeric',
      month: 'short',
      day: 'numeric',
    });
  };

  return (
    <div className="space-y-5">
      {error && <Alert type="error" title="Error" message={error} onClose={clearError} />}
      {invalidRefError && (
        <Alert
          type="error"
          title="Invalid Flow Reference"
          message={invalidRefError}
          onClose={() => setInvalidRefError(null)}
        />
      )}

      {/* Header */}
      <div className="flex items-end justify-between">
        <div>
          <p className="font-mono text-[10px] uppercase tracking-widest mb-1" style={{ color: '#A8A6B0' }}>Flows</p>
          <h1 className="font-serif text-[28px] leading-tight" style={{ color: '#03031F', fontWeight: 400 }}>Conversation Flows</h1>
        </div>
        <Button variant="primary" onClick={() => navigate('/flows/new')} leftIcon={<Plus size={15} />}>
          Create New Flow
        </Button>
      </div>

      {/* Flow List */}
      <Card>
        <CardHeader
          title="Your Flows"
          subtitle="Manage conversation flows for your business"
        />
        <CardBody>
          {isLoading && flows.length === 0 ? (
            <div className="flex items-center justify-center py-12">
              <div className="spinner" style={{ width: 24, height: 24 }} />
            </div>
          ) : flows.length === 0 ? (
            <div className="text-center py-12">
              <p className="text-[14px]" style={{ color: '#6F6D7A' }}>
                No flows yet. Create your first conversation flow to get started.
              </p>
              <Button
                variant="primary"
                className="mt-4"
                onClick={() => navigate('/flows/new')}
                leftIcon={<Plus size={15} />}
              >
                Create New Flow
              </Button>
            </div>
          ) : (
            <div className="space-y-3">
              {flows.map((flow) => (
                <div
                  key={flow.id}
                  className="flex items-center justify-between p-4 rounded-salex-md border transition-colors hover:bg-gray-50"
                  style={{ borderColor: '#E5E4E3' }}
                >
                  {/* Flow info */}
                  <div className="flex items-center gap-4 min-w-0">
                    <div className="min-w-0">
                      <p className="font-semibold text-salex-sm truncate" style={{ color: '#03031F' }}>
                        {flow.name}
                      </p>
                      <div className="flex items-center gap-3 mt-1">
                        <span className="font-mono text-[11px]" style={{ color: '#A8A6B0' }}>
                          v{flow.version}
                        </span>
                        <span className="font-mono text-[11px]" style={{ color: '#A8A6B0' }}>
                          Updated {formatDate(flow.updatedAt)}
                        </span>
                      </div>
                    </div>
                  </div>

                  {/* Status + Actions */}
                  <div className="flex items-center gap-3 flex-shrink-0">
                    <Badge
                      variant={flow.isActive ? 'success' : 'muted'}
                      label={flow.isActive ? 'Active' : 'Inactive'}
                      dot
                    />

                    {/* Activate / Deactivate toggle */}
                    {flow.isActive ? (
                      <Button
                        size="sm"
                        variant="secondary"
                        onClick={() => handleDeactivate(flow.id)}
                        disabled={isSaving}
                        title="Deactivate flow"
                      >
                        <PowerOff size={14} />
                      </Button>
                    ) : (
                      <Button
                        size="sm"
                        variant="secondary"
                        onClick={() => handleActivate(flow.id)}
                        disabled={isSaving}
                        title="Activate flow"
                      >
                        <Power size={14} />
                      </Button>
                    )}

                    {/* Edit */}
                    <Button
                      size="sm"
                      variant="secondary"
                      onClick={() => navigate(`/flows/${flow.id}/edit`)}
                      title="Edit flow"
                    >
                      <Pencil size={14} />
                    </Button>

                    {/* Delete */}
                    <Button
                      size="sm"
                      variant="danger"
                      onClick={() => handleDeleteClick({ id: flow.id, name: flow.name, isActive: flow.isActive })}
                      disabled={flow.isActive}
                      title={flow.isActive ? 'Deactivate before deleting' : 'Delete flow'}
                    >
                      <Trash2 size={14} />
                    </Button>
                  </div>
                </div>
              ))}
            </div>
          )}
        </CardBody>
      </Card>

      {/* Delete Confirmation Modal */}
      <Modal
        isOpen={!!deleteTarget}
        onClose={() => setDeleteTarget(null)}
        title="Delete Flow"
        footer={
          <>
            <Button variant="secondary" onClick={() => setDeleteTarget(null)}>
              Cancel
            </Button>
            <Button
              variant="danger"
              onClick={handleConfirmDelete}
              isLoading={isSaving}
              disabled={deleteTarget?.isActive}
            >
              Delete
            </Button>
          </>
        }
      >
        {deleteTarget && (
          <div className="space-y-3">
            {deleteTarget.isActive ? (
              <Alert
                type="warning"
                title="Cannot delete active flow"
                message="You must deactivate this flow before it can be deleted."
              />
            ) : (
              <p className="text-[13px]" style={{ color: '#2E2D38' }}>
                Are you sure you want to delete <strong>{deleteTarget.name}</strong>? This action cannot be undone.
              </p>
            )}
          </div>
        )}
      </Modal>
    </div>
  );
};
