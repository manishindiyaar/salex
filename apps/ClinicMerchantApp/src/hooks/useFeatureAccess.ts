...
export function useFeatureAccess(featureCode: string): FeatureAccessResult {
  ...
  const checkAccess = useCallback(async () => {
    ...
    try {
      ...
      // For V1, plan-based gating is bypassed. All core features are allowed if the subscription is TRIAL or ACTIVE.
      const isActive = subscription.status === 'TRIAL' || subscription.status === 'ACTIVE';

      setResult({
        allowed: isActive,
        reason: isActive ? undefined : `Subscription is ${subscription.status?.toLowerCase() || 'inactive'}`,
        loading: false,
      });
    }
