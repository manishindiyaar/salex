...
const ServicesPricingScreen: React.FC<ServicesPricingScreenProps> = ({ navigation }) => {
  ...
  // Initialize with existing draft services or CLINIC_CONFIG defaults
  const getInitialServices = (): ServiceDraft[] => {
    ...
    return CLINIC_CONFIG.defaultServices.map((service, index) => ({
      id: (index + 1).toString(),
      name: service.name,
      price: service.price,
  ...
  // Quick-add templates — always Salon services from clinicConfig
