...
const ContactLocationScreen: React.FC<ContactLocationScreenProps> = ({ navigation }) => {
  ...
  // Form state
  const [phoneNumber, setPhoneNumber] = useState(businessDraft.phone || userPhone || '');
  const [whatsappNumber, setWhatsappNumber] = useState(businessDraft.whatsApp || userPhone || '');
  const [country, setCountry] = useState(businessDraft.address?.country || 'India');
