...
class SubscriptionService {
    ...
    async createTrialSubscription(input) {
        ...
        const existingSubscription = await shared_types_1.prisma.subscription.findUnique({
            where: { businessId },
        });
        if (existingSubscription) {
            throw new errors_1.BusinessRuleError('Business already has a subscription');
        }
        ...
        const subscription = await shared_types_1.prisma.subscription.create({
            data: {
                businessId,
                plan,
                status: 'TRIAL',
        ...
        logger_1.logger.info({ businessId, subscriptionId: subscription.id, plan, trialEndsAt }, 'Trial subscription created');
        return subscription;
    }
    /**
    ...
    async getByBusinessId(businessId) {
        ...
        if (!subscription) {
            throw new errors_1.NotFoundError('Subscription not found for this business');
        }
        return subscription;
    }
    /**
    ...
    async transitionStatus(input) {
        ...
        if (!allowedTransitions.includes(newStatus)) {
            throw new errors_1.BusinessRuleError(`Invalid transition from ${currentStatus} to ${newStatus}. ` +
        ...
        const updated = await shared_types_1.prisma.subscription.update({
            where: { id: subscriptionId },
            data: updateData,
        });
    ...
    async getAll(status) {
        ...
        return shared_types_1.prisma.subscription.findMany({
            where,
            include: {
                business: {
                    select: {
    ...
    async processExpiredSubscriptions() {
        const now = new Date();
        // Find expired trials
        const expiredTrials = await shared_types_1.prisma.subscription.findMany({
            where: {
                status: 'TRIAL',
                trialEndsAt: {
                    lte: now,
        ...
        const expiredGrace = await shared_types_1.prisma.subscription.findMany({
            where: {
                status: 'GRACE',
                currentPeriodEnd: {
                    lte: now,
        ...
        if (totalExpired > 0) {
            logger_1.logger.info({ expiredTrials: expiredTrials.length, expiredGrace: expiredGrace.length }, `Processed ${totalExpired} expired subscriptions`);
        }
    }
    /**
     * Update subscription plan
     */
    async updatePlan(subscriptionId, newPlan) {
        const subscription = await this.getById(subscriptionId);
        const updated = await shared_types_1.prisma.subscription.update({
            where: { id: subscriptionId },
            data: { plan: newPlan },
