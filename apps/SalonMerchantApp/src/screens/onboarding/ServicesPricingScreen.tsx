...
const ServicesPricingScreen: React.FC<ServicesPricingScreenProps> = ({ navigation }) => {
  ...
  // Initialize with existing draft services or SALON_CONFIG defaults
  const getInitialServices = (): ServiceDraft[] => {
    ...
    // Pre-populate with Salon default services — no API call needed
    return SALON_CONFIG.defaultServices.map((service, index) => ({
      id: (index + 1).toString(),
      name: service.name,
      price: service.price,
  ...
  // Quick-add templates — always Salon services from salonConfig
  const getServiceTemplates = (): ServiceDraft[] => {
    return SALON_CONFIG.defaultServices.map((service, index) => ({
      id: `salon-${index}`,
      name: service.name,
      price: service.price,
  ...
  const handleAddTemplate = (template: ServiceDraft) => {
    ...
    setServices([...services, serviceDraft]);
  };
          ...
          >
            ...
            <View style={styles.section}>
              ...
              <Text style={styles.sectionDescription}>
                Popular services for Hair Salons
              </Text>
              <FlatList
